...
void tft_t::putch(char ch)
{
	...
	bool yTransform = transform() && yy < bottomEdge() && yy + yStop - yStart > bottomEdge();
	area(x(), yy, w, h);
#else
	area(x(), y(), w, h);
#endif
...
#endif
		unsigned char c = pgm_read_byte(&(ascii[(uint8_t)ch - ' '][yi / zoom()]));
...
disp:
	...
	start();
#ifdef TFT_VERTICALSCROLLING
	bool yTransform = transform() && y < bottomEdge() && y + h > bottomEdge();
#endif
